...
(
    ...
    `CSRPCONT`  VARCHAR(1000)    NOT NULL, 
    `CSID`      INT              NOT NULL, 
    `MEMID`     VARCHAR(15)      NOT NULL, 
    PRIMARY KEY (CSRPID)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
(
    ...
    `STRPREC`   INT             NOT NULL, 
    `STRPREJ`   INT             NOT NULL, 
    `STRID`     INT             NOT NULL, 
    `MEMID`     VARCHAR(15)     NOT NULL, 
    PRIMARY KEY (STRPID)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
ALTER TABLE CART ADD CONSTRAINT FK_CART_MAPID_STRUMF_MAPID FOREIGN KEY (MAPID)
 REFERENCES STRUMF (MAPID)  ON DELETE RESTRICT ON UPDATE RESTRICT;
